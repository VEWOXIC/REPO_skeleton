...
def get_dataset(cfg, flag):
    # 可能要选择 
    if cfg['model']['model_name'] in ["Autoformer", "Transformer", "Informer", "Reformer", "FEDformer"]:
        return Dataset_Transformer(cfg, flag)
    else:
...
class Dataset_Transformer(Dataset):
    def __init__(self, cfg, flag) -> None:
        ...
        self.flag = flag
        self.lookback = cfg['data']['lookback']
        self.horizon = cfg['model']['pred_len'] + cfg['model']['label_len']
        self.timeStampFreq = cfg['data']['freq'] # Choose a time feature
        self.normalize = cfg['data']['normalize']
