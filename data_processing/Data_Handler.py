...
class Dataset_Traffic(Dataset):
    ...
    def __init__(self, cfg, flag):
        ...
        self.load_external = cfg['model']['load_external']
        # self.normal_external = self.config.get('normal_external', False)
        self.add_time_in_day = cfg['model']['add_time_in_day']
        self.add_day_in_week = cfg['model']['add_day_in_week']
        ...
        self.data_path = './datasets/' + self.dataset + '/'
        if not os.path.exists(self.data_path):
            raise ValueError("Dataset {} not exist! Please ensure the path "
                             "'./datasets/{}/' exist!".format(self.dataset, self.dataset))
        # 加载数据集的config.json文件
        self.weight_col = cfg['data']['weight_col']
        self.data_col = cfg['data']['data_col']
        self.ext_col = cfg['data']['ext_col']
        self.geo_file = cfg['data']['geo_file']
        self.rel_file = cfg['data']['rel_file']
        self.data_files = cfg['data']['data_files']
        self.ext_file = cfg['data']['ext_file']
        self.output_dim = cfg['data']['output_dim']
        self.time_intervals = cfg['data']['time_intervals']
        self.init_weight_inf_or_zero = cfg['data']['init_weight_inf_or_zero']
        self.set_weight_link_or_dist = cfg['data']['set_weight_link_or_dist']
        self.bidir_adj_mx =cfg['data']['bidir_adj_mx']
        self.calculate_weight_adj = cfg['data']['calculate_weight_adj']
        ...
        if self.data is None:
            ...
            if os.path.exists(self.cache_file_name):
                self.data_x, self.data_y = self._load_cache()
            else:
    ...
    def _load_dyna(self, filename):
        ...
        """
        return self._load_dyna_3d(filename)
    ...
    def _add_external_information_3d(self, df, ext_data=None):
        ...
        Returns:
            np.ndarray: 融合后的外部数据和交通状态数据, (len_time, num_nodes, feature_dim_plus)
        """
        print("_add_external_information_2d")
    ...
    def _add_external_information_4d(self, df, ext_data=None):
        print("_add_external_information_4d")
        """
        增加外部信息（一周中的星期几/day of week，一天中的某个时刻/time of day，外部数据）
    ...
    def __getitem__(self, index):
        ...
        if self.cfg['model']['UseTimeFeature']:
            timestamp_x = self.data_stamp[index:index+self.input_window]
            timestamp_y = self.data_stamp[index+self.input_window:index+self.input_window+self.output_window]
        else:
            timestamp_x = 0
            timestamp_y = 0
    ...
    def _load_cache(self):
        ...
        print('Loading ' + self.cache_file_name)
